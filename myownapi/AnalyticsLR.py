...
class AnalyticsLR(MainAPI):
    ...
    def aplicarRegressaoLinear(self):
        ...
        self.info("Iniciando regressão Linear!")
        self.lreg = LinearRegression()  # Instanciando um novo objeto [sklearn.linear_model]
        self.lreg.fit(self.x_train, self.y_train);  # Treinando os X e Y
        self.y_pred = self.lreg.predict(self.x_test);  # Realiza a predição de acordo com o X de teste
        self.info("Regressão Linear aplicada com sucesso!")
    ...
    def calcularCoeficientesDePesos(self):
        ...
        self.dfDePesos['Coefficient'] = Series(
            self.lreg.coef_)  # O LinearRegression() Salva esses coeficientes na variavel "coef_"
