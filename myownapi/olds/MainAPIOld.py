...
class MainAPI:
    ...
    @staticmethod
    def arimaDefinirColunaObjetivo(nomeDaColunaObjetivo, nomeDaColunaDeDatas):
        MainAPI.df[nomeDaColunaDeDatas] = pd.to_datetime(MainAPI.df[nomeDaColunaDeDatas])
        MainAPI.df = MainAPI.df.groupby(nomeDaColunaDeDatas)[nomeDaColunaObjetivo].sum().reset_index()
        MainAPI.df = MainAPI.df.set_index(nomeDaColunaDeDatas)
